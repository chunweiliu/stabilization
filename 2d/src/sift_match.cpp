...
int main( int argc, char *argv[] ) 
{
	...
	std::cerr << "Usage: " << argv[0]
	    << " <input_key_dir1> <input_key_dir2> <output_key_dir1> <output_key_dir2>"
	    << std::endl;
	return -1;
    }
    ...
    parseFileName( argv[1], image_name1, image_postfix1 );
    
    char image_postfix2;
    char image_name2[128];
    parseFileName( argv[2], image_name2, image_postfix2 );

    FeatureListLoader feature_list_loader1( input_key_dir1, image_name1, image_postfix1 );
    FeatureListLoader feature_list_loader2( input_key_dir2, image_name2, image_postfix2 ); // for deshaker
	...
	sprintf( output_file1, "%s%s_%04d_%c.key", output_key_dir1, image_name1, frame_num, image_postfix1 );
	sprintf( output_file2, "%s%s_%04d_%c.key", output_key_dir2, image_name2, frame_num, image_postfix2 ); // for deshaker
	//sprintf( output_fileF, "%s%s_%04d.fund", output_fund_mat_dir, image_name, frame_num );
#ifdef NO_OVERLAP
    
	if ( access( output_file1, F_OK )==0 && 
	     access( output_file2, F_OK )==0 ) continue;
#endif
	...
	writeKeyFile( output_file1, matched_list1 );
	writeKeyFile( output_file2, matched_list2 );
