...
int main( int argc, char *argv[] ) 
{
    if (argc != 6) {
	cerr << "Usage: " << argv[0]
	     << " <input_image_dir> <tracks_file> <output_image_file> <plot_tracks_length> <background_iamge_num>"
	     << endl;
	return -1;
    }
    ...
    const char* plot_tracks_length = argv[4];
    const char* background_image_num = argv[5];
    
    PLOT_TRACKS_LENGTH = atoi(plot_tracks_length);
    const int BACKGROUND_IMAGE_NUM = atoi(background_image_num);
    ...
    char image_file[128];
    sprintf( image_file, "%s%s_%04d_%c.png", image_dir, image_name, BACKGROUND_IMAGE_NUM, image_postfix );
    IplImage *frame = cvLoadImage( image_file );
    IplImage *output_frame = cvCloneImage( frame );

    // read and draw tracks file
    plotTracks( tracks_file, output_frame );

    cvSaveImage( output_file, output_frame );
